...
const BigFacilityCard = ({fac}) => {

  const [isLiked, setIsLiked] = useState(fac.isLiked);
  const router = useRouter();

  const handleClick = () => {
    router.push(`/place/placedetail?placeId=${fac.id}`);
  };
  ...
  const handleFavoriteClick = async (e) => {
    ...
    try {
      if (isLiked) {
        await removeFavorite(fac.id);
      } else {
        await addFavorite(fac.id);
      }
      // 상태 업데이트
  ...
  return (
    ...
    <Container onClick={handleClick}>
      <ImageWrapper>
        ...
        <FavoriteButton onClick={handleFavoriteClick} $isLiked={isLiked}>
...
const FavoriteButton = styled.button.withConfig({
  shouldForwardProp: (prop) => !["hovered", "$isliked"].includes(prop), // isLiked prop을 DOM에 전달하지 않음
})`
  position: absolute;
  top: 10px;
  right: 10px;
